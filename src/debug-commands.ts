...
/**
 * Register the three interrogate debug commands (P1.M2.T3.S1). Called once
 * by index.ts at factory time with the SAME loaded config the tool uses —
 * never reload it here.
 *
...
export function registerDebugCommands(
  ...
  pi.registerCommand("interrogate-debug-upsert", {
    description:
      "Debug: upsert questions through the interrogate tool's exact code path (parse → guards → caps → merge). Usage: /interrogate-debug-upsert {\"goal\":\"...\",\"questions\":[...]}",
  ...
  pi.registerCommand("interrogate-debug-state", {
    description:
      "Debug: print the interrogation status line and one line per question (id, status, rev, prompt preview, answer).",
